...
export default function Rooms() {
  ...
  const getOccupantNames = (room: Room) => {
    if (!workers || workers.length === 0) {
      return room.listeOccupants.map(() => 'Chargement...');
    }

    const validWorkers = [];
    const invalidIds = [];

    for (const occupantId of room.listeOccupants) {
      ...
      if (worker) {
        // Check if worker is active (no exit date or exit date in future)
        const isActive = !worker.dateSortie || new Date(worker.dateSortie) > new Date();
        if (isActive) {
          validWorkers.push(worker);
        } else {
          invalidIds.push(occupantId);
        }
      } else {
        invalidIds.push(occupantId);
      }
    ...
    // If there are invalid IDs, clean up the room data asynchronously
    if (invalidIds.length > 0) {
      console.log(`🧹 Found ${invalidIds.length} invalid occupant IDs in room ${room.numero}:`, invalidIds);
      cleanupRoomOccupants(room, invalidIds);
    }

    return validWorkers.map(worker => worker.nom);
  };

  // Async function to clean up invalid occupant IDs
  const cleanupRoomOccupants = async (room: Room, invalidIds: string[]) => {
    try {
      const validOccupants = room.listeOccupants.filter(id => !invalidIds.includes(id));
      const roomRef = doc(db, 'rooms', room.id);

      await updateDoc(roomRef, {
        listeOccupants: validOccupants,
        occupantsActuels: validOccupants.length,
        updatedAt: new Date()
      });

      console.log(`✅ Cleaned up room ${room.numero}: removed ${invalidIds.length} invalid occupant(s)`);
    } catch (error) {
      console.error(`❌ Failed to cleanup room ${room.numero}:`, error);
    }
  };

  // Function to manually sync all room occupancy
          ...
          <div className="flex items-center space-x-3 mt-2">
            <div className="space-y-1">
              ...
              <p className="text-sm text-blue-600 font-medium">
                {(() => {
                  const totalCapacity = filteredRooms.reduce((total, room) => total + room.capaciteTotale, 0);
                  const actualOccupied = filteredRooms.reduce((total, room) => {
                    const actualOccupants = workers.filter(worker =>
                      worker.chambre === room.numero &&
                      worker.fermeId === room.fermeId &&
                      worker.statut === 'actif' &&
                    ...
                    return total + actualOccupants;
                  }, 0);
                  const actualFree = totalCapacity - actualOccupied;
                  return `Total places: ${totalCapacity} • Occupées: ${actualOccupied} • Libres: ${actualFree}`;
                })()}
              </p>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {filteredRooms.reduce((total, room) => total + room.capaciteTotale, 0)}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">
                  {(() => {
                    // Calculate actual occupancy from worker data
                    return filteredRooms.reduce((total, room) => {
                      const actualOccupants = workers.filter(worker =>
                        worker.chambre === room.numero &&
                        worker.fermeId === room.fermeId &&
                        worker.statut === 'actif' &&
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">
                  {(() => {
                    ...
                    return totalCapacity - actualOccupied;
                  })()}
                </p>
              </div>
                      ...
                      <div className="space-y-1">
                        {occupantNames.length > 0 ? (
                          ...
                          <div className="text-sm text-gray-500 italic px-2 py-1">
                            {workers && workers.length > 0 ? 'Aucun occupant valide trouvé' : 'Chargement des occupants...'}
                          </div>
                        )}
                      </div>
                    </div>
