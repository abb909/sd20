...
// Exit reason translation function
const getMotifLabel = (motif: string): string => {
  ...
  return motifTranslations[motif] || motif || 'Non spécifié';
};
...
export default function Statistics() {
  ...
  const generateAdvancedPDFReport = async (fermeId: string | 'all' = 'all') => {
    try {
      ...
      const exitAnalysisData = [
        ...
        ['Motif Principal', getMotifLabel(statistics.topExitReason), `${statistics.topExitReasonCount} cas`, 'Analyser et corriger cause principale'],
      ...
      // Farm comparison table will be moved to the end
      ...
      const exitReasonsData = [
        ...
        ...Object.entries(exitReasons)
          ...
          .map(([reason, count]) => [
            ...
            count.toString(),
            totalExited > 0 ? `${Math.round((count / totalExited) * 100)}%` : '0%',
            count > (totalExited * 0.2) ? 'Fréquent' : count > (totalExited * 0.1) ? 'Occasionnel' : 'Rare'
          ])
      ];
      ...
      (doc as any).autoTable({
        startY: yPosition,
        head: [exitReasonsData[0]],
        body: exitReasonsData.slice(1),
        theme: 'striped',
        headStyles: {
      ...
      const supervisorStats = supervisors.map(supervisor => {
        ...
        return {
          supervisor,
          workerCount: workersUnderSupervisor.length
        };
      }).sort((a, b) => b.workerCount - a.workerCount);

      // Add workers without supervisor
      const workersWithoutSupervisor = activeWorkers.filter(w => !w.supervisorId);

      const supervisorTableData = [
        ['Superviseur', 'Téléphone', 'Nombre d\'ouvriers', 'Pourcentage', 'Performance'],
        ...supervisorStats.map(({ supervisor, workerCount }) => [
          supervisor.nom,
          supervisor.telephone,
          workerCount.toString(),
          activeWorkers.length > 0 ? `${Math.round((workerCount / activeWorkers.length) * 100)}%` : '0%',
          workerCount > 10 ? 'Excellent' : workerCount > 5 ? 'Bon' : workerCount > 0 ? 'Acceptable' : 'Inactif'
        ]),
        workersWithoutSupervisor.length > 0 ? [
          'Sans superviseur',
          '-',
          workersWithoutSupervisor.length.toString(),
          activeWorkers.length > 0 ? `${Math.round((workersWithoutSupervisor.length / activeWorkers.length) * 100)}%` : '0%',
          'À assigner'
        ] : null
  ...
  const generateBasicPDFReport = async (fermeId: string | 'all' = 'all') => {
    ...
    }).filter(stat => stat.workerCount > 0).sort((a, b) => b.workerCount - a.workerCount);
    ...
    // Add "Sans superviseur" row if there are unassigned workers
    if (workersWithoutSupervisor.length > 0) {
      supervisorTableData.push([
        'Sans superviseur',
        '-',
        workersWithoutSupervisor.length.toString(),
        activeWorkers.length > 0 ? `${Math.round((workersWithoutSupervisor.length / activeWorkers.length) * 100)}%` : '0%'
    ...
    // ADD FARM COMPARISON TABLE AT THE END (Basic PDF)
    if (fermeId === 'all') {
      doc.addPage();
      yPosition = margin;
